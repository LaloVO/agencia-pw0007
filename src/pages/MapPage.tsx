...
const MapPage = () => {
  ...
  // Mapbox Token resolved exclusively from useSiteUser/fallback (no import.meta.env)
  const mapboxToken = (
  ...
  return (
    <>
      <Helmet>
        <title>Explorar Propiedades | {siteName}</title>
        <meta
          name="description"
          content={`Explora el catálogo completo de propiedades de ${siteName} en Saltillo, Coahuila.`}
        />
      </Helmet>

      <Navbar />

      <main className="pt-[72px] h-screen flex overflow-hidden relative bg-background select-none">
        ...
        <div className={cn(
          ...
          <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 max-w-[95vw]">
            ...
            />
            {(qParam || actionParam) && (
              <div className="flex flex-wrap gap-1.5 bg-background border border-foreground/[0.08] p-2 rounded-2xl shadow-elegant w-fit max-w-[90vw]">
                {qParam && (
                  <span className="flex items-center gap-1.5 bg-foreground text-background text-[10px] font-sans font-bold uppercase tracking-wider px-3 py-1 rounded-full">
                    Búsqueda: {qParam}
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="hover:opacity-70 transition-opacity ml-1 font-extrabold text-xs"
                    >
                      ×
                    </button>
                  </span>
                ...
                {actionParam && (
                  <span className="flex items-center gap-1.5 bg-foreground/10 text-foreground text-[10px] font-sans font-bold uppercase tracking-wider px-3 py-1 rounded-full border border-foreground/20">
                    Operación: {actionParam === '1' ? 'Venta' : 'Renta'}
                    <button
                      type="button"
                      onClick={() => {
                        const newParams = new URLSearchParams(searchParams);
                ...
                <button
                  ...
                  className="text-[9px] uppercase tracking-widest font-extrabold text-foreground/40 hover:text-foreground px-2 py-1 transition-colors"
                >
                  Limpiar
                </button>
              </div>
        ...
        <aside className={cn(
          "flex-col w-full lg:w-96 border-l border-foreground/[0.06] bg-background shrink-0 h-full",
          viewMode === 'list' ? 'flex' : 'hidden lg:flex'
        )}>
          <div className="px-5 py-4 border-b border-foreground/[0.06]">
            <h1 className="font-sans font-extrabold text-lg uppercase tracking-wider text-foreground">Propiedades</h1>
            <p className="text-[10px] text-foreground/40 uppercase tracking-widest font-bold mt-1">
              {isLoading ? 'Cargando…' : `${filtered.length} resultado${filtered.length !== 1 ? 's' : ''}`}
            </p>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24 lg:pb-4 hide-scrollbar bg-background">
            {filtered.map((p) => (
            ...
            {!isLoading && filtered.length === 0 && (
              <div className="text-center py-16">
                <p className="font-sans text-lg text-foreground/40 font-medium">Sin resultados</p>
                <p className="text-xs text-foreground/30 mt-2">Prueba cambiando tus filtros de búsqueda</p>
              </div>
            )}
          </div>
        </aside>
        ...
        <button
          ...
          className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 px-5 py-3 rounded-full bg-foreground text-background font-sans text-xs font-bold uppercase tracking-wider shadow-elegant hover:opacity-90 active:scale-95 transition-all cursor-pointer"
        >
          {viewMode === 'map' ? (
            <>
              <List className="w-4 h-4 text-background" />
              <span>Ver Lista</span>
            </>
          ) : (
            <>
              <MapIcon className="w-4 h-4 text-background" />
              <span>Ver Mapa</span>
            </>
          )}
        </button>
